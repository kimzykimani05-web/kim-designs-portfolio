...
const TestimonialsSection = () => {
  ...
  return (
    <section id="testimonials" className="relative py-24 lg:py-32 bg-dark-primary overflow-hidden">
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-1/4 left-0 w-72 h-72 bg-brand-purple/10 rounded-full blur-3xl"></div>
        <div className="absolute bottom-1/4 right-0 w-72 h-72 bg-brand-cyan/10 rounded-full blur-3xl"></div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="md:hidden">
          <motion.div
            ref={containerRef}
            className="flex gap-6 overflow-x-auto snap-x snap-mandatory scrollbar-hide px-6 -mx-6"
            drag="x"
          ...
          >
            {testimonials.map((t, idx) => (
              ...
              >
                <div className="glass-card rounded-3xl p-8 h-full flex flex-col card-hover relative">
                  <div className="text-5xl text-brand-cyan/30 mb-4 leading-none">"</div>
                  ...
                  <p className="text-light-secondary leading-relaxed mb-8 flex-grow italic text-lg">"{t.quote}"</p>
                  
                  <div className="flex items-center space-x-4 pt-6 border-t border-white/10">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-brand-cyan/20 to-brand-purple/20 flex items-center justify-center text-lg font-bold text-brand-cyan">
                      {t.initials}
                    </div>
                    <div>
                      <p className="font-bold text-light-primary">{t.author}</p>
                      <p className="text-sm text-light-muted">{t.role}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((t, idx) => (
            <motion.div
              ...
              className="group"
            >
              <div className="glass-card rounded-3xl p-8 lg:p-10 h-full flex flex-col card-hover relative">
                <div className="text-5xl text-brand-cyan/20 mb-4 leading-none">"</div>
                ...
                <p className="text-light-secondary leading-relaxed mb-8 flex-grow italic">"{t.quote}"</p>
                ...
                <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-brand-cyan/5 to-brand-purple/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
              </div>
            </motion.div>
          ))}
        </div>
