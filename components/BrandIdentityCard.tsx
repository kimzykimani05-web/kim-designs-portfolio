...
export default function BrandIdentityCard({ project, index = 0 }: BrandIdentityCardProps) {
  ...
  return (
    ...
    >
      <div className="glass-card rounded-3xl overflow-hidden card-hover h-full flex flex-col brand-card-accent">
        ...
        <div className="p-5 sm:p-6 lg:p-8 flex-1 flex flex-col">
          <h3 className="text-xl sm:text-2xl font-bold text-light-primary mb-2 group-hover:text-brand-cyan transition-colors">
            {project.clientName}
          </h3>
          {project.projectTitle && (
            <p className="text-sm text-brand-cyan font-medium mb-3">{project.projectTitle}</p>
          )}
          ...
          <Link
            href={`/portfolio/${project.slug}`}
            className="inline-flex items-center gap-1.5 text-sm font-medium text-brand-cyan hover:text-brand-purple transition-colors min-h-[44px]"
          >
            View Case Study
