...
export default function WebsiteCard({ project, index = 0 }: WebsiteCardProps) {
  ...
  return (
    ...
    >
      <div className="glass-card rounded-3xl overflow-hidden card-hover h-full flex flex-col website-card-accent">
        <div className="relative aspect-[16/10] sm:aspect-[16/10] overflow-hidden bg-dark-card">
          {primaryImage ? (
            <Image
              src={primaryImage}
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-brand-cyan/20 via-dark-card to-brand-blue/20" />
          )}
          ...
          <div className="absolute top-3 left-3">
            <span className="px-3 py-1.5 rounded-full bg-dark-card/90 backdrop-blur-sm text-xs font-semibold text-brand-cyan border border-brand-cyan/20">
              Website
            </span>
          </div>
          ...
          <div className="absolute inset-0 flex items-end p-4 sm:p-5 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            <div className="flex flex-wrap gap-2">
              {project.liveUrl && (
                <a
                  ...
                  className="btn-premium text-[10px] sm:text-sm py-2 px-3 sm:py-2.5 sm:px-5"
                >
                  Visit Live
              ...
              <Link
                ...
                className="btn-outline-premium text-[10px] sm:text-sm py-2 px-3 sm:py-2.5 sm:px-5"
              >
                Case Study
              </Link>
            </div>
        ...
        <div className="p-5 sm:p-6 lg:p-8 flex-1 flex flex-col">
          ...
          <Link
            href={`/portfolio/${project.slug}`}
            className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-medium text-brand-cyan hover:text-brand-purple transition-colors min-h-[44px]"
          >
            View Case Study
